...
def disp_noise(img, flux_max=2, control_plot=False):
    ...
    if control_plot:
        ...
        ax.plot(x_new, fit_hist, color='red', label='Gaussian Fit\nµ = {:.2e} DN/s\n σ = {:.2e} DN/s'.format(popt[1], abs(popt[2])))
        ...
        ax.legend()
        #fig.tight_layout()
        fig.savefig(fig_dir + 'histBgd_'+ name_file[11:] +'.pdf', dpi=300)
        #plt.show()
...
def bgd_noise_removal(rate_files, max_count_value = 0.5, threshold_factor = 1.5, saveplot = False):
    ...
    if not os.path.exists(input_dir):   # The input directory do not exist
        print(
            'The \'input_files\' directory does not exist. Please create this directory and insert the files you want to correct.')
        ...
        if len(rate_files) == 0:
            print(
                'No files found. Make sure the files have the \'_rate\' suffix and the \'.fits\' extension. Also make sure the files are placed in the \'input_files\' directory.')
        else:
            ...
            if len(set(instrumes_list)) == 1:           # All the files come from the same instrument
                ...
                for n, rate_file in enumerate(tqdm(rate_files)):
                    ...
                    sigNoise = disp_noise(imgNotCorrected, flux_max = maxFlux, control_plot = False)

                    if instrume == instrumes_expected[0]:       # NIRSPEC
                        meansArray, imgCorrected = corr_img_NIRSpec(imgNotCorrected, threshold = thresholdFactor*sigNoise, return_means = True)
                    elif instrume == instrumes_expected[1]:     # NIRCAM
                        meansArray, imgCorrected = corr_img_NIRCam(imgNotCorrected, threshold = thresholdFactor*sigNoise, return_means = True)

                    sci_hdu.data = imgCorrected

                    hdul = fits.HDUList([primary_hdu, sci_hdu, err_hdu, dq_hdu, varPoisson_hdu, varRnoise_hdu, asdf_hdu])
                    ...
                    if saveplot:
                        ...
                        im = mainAx.imshow(imgNotCorrected, origin='lower', cmap='Greys', norm=colors.LogNorm(vmin=minNoCorr, vmax=vmax))
                        ...
                        im = mainAx.imshow(imgCorrected, origin='lower', cmap='Greys', norm=colors.LogNorm(vmin=minNoCorr, vmax=vmax))
                        ...
                        im = ax.imshow(meanImg, origin='lower', cmap='Greys', vmin=np.nanmin(meansArray), vmax=np.nanmax(meansArray))
...
maxFlux = 0.5           # DN/s
thresholdFactor = 1.5   # Factor to apply on the dispersion value used to estimate a flux median value: σ-clip value = thresholdFactor * σ_dispersion
